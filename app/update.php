<?php
	...
	if(empty($_SESSION['username']))
		...
		if(isset($_POST['register_individuals'])) {
			...
			$id = $_POST['id'];
			try {
				$stmt = $connect->prepare('UPDATE room_rental_registrations SET fullname = ?,  email = ?, mobile = ?, alternat_mobile = ?, plot_number = ?, rooms = ?, country = ?, state = ?, city = ?, address = ?, landmark = ?, rent = ?, deposit = ?, description = ?, accommodation = ?, other = ?, vacant = ?, user_id = ?  WHERE id = ?');
				$stmt->execute(array(
					$fullname,
					$email,
					$mobile,
